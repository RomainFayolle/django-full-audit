...
def coverage(c):
    ...
    """
    c.run("coverage run --source django_full_audit runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django_full_audit.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_full_audit")
...
def lint(c):
    ...
    """
    c.run("flake8 django_full_audit tests")
...
def release(c, bumpsize=''):
    ...
    import django_full_audit
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_full_audit.__version__))
    c.run("git push --tags")
